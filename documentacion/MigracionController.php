        ...
        while (!feof($file)) {
            ...
            if (count($elementos) == 54) {
                try {
                    ...
                    // $nombreClasificacion = explode("^", $elementos[1]);
                    // if (count($nombreClasificacion) > 1) {
                    //     $nombreClasificacion = substr($nombreClasificacion[1], 1, strlen($nombreClasificacion[1]) - 1);
                    // } else {
                    //     $nombreClasificacion = '';
                    // }

                    $nombreClasificacion = str_replace("^a","//",$elementos[1]);
                    $nombreClasificacion = str_replace("^b","//",$nombreClasificacion);
                    $nombreClasificacion = str_replace("^c","//",$nombreClasificacion);

                    $clasificacion = Clasificacion::where('nombre', $nombreClasificacion)->get()->first();  /////////////^a
